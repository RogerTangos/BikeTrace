...
- (void) turnOffTrackingAndRemoveCrumbs:(double)seconds {
    ...
    //add the ride to the sql ride table.
    ...
    //actually insert the ride data into the sql.
    [self.ride insertIntoSQL:[rideDirectory getDBPath]];
    
    NSLog(@"About to call [ride postToServer]");
    [self.ride postToServer];
    
    // JSONIFY data here. (call function in Ride)
    // ship to API here.
...
//            NSLog(@"Tracking was on.");
        ...
        //you weren't tracking. Just cut the power.
        else{
//            NSLog(@"The App closed.");
//            NSLog(@"Background mode was off.");
